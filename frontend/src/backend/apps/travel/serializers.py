...
class ItinerarySerializer(serializers.ModelSerializer):
    """조회 + 생성/수정(중첩 days/items 포함)을 함께 처리한다.

    M004-F-001 최종 일정표 생성, M004-F-002 일자별 일정 관리,
    M004-F-003 일정 수정, M004-F-004 직접 일정 추가를 하나의 엔드포인트로 지원.
    """
    ...
    def _sync_days(itinerary, days_data):
        """전달된 days/items로 전체를 교체한다 (단순하고 예측 가능한 저장 방식)."""
        if not days_data:
            return
...
class ItineraryRouteSerializer(serializers.Serializer):
    """M004-F-006 여행 경로 지도 표시용 — 일자별 순서대로의 좌표 목록."""
...
class ItineraryShareSerializer(serializers.Serializer):
    """POST /itineraries/{id}/share/ 응답 형태 (문서화 전용)."""
