...
class ItinerarySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = (
            ...
            "booked_product_type", "booked_package_db_id", "booked_price", "booked_package_name",
            "hotel", "thumbnail_url",
        )
    ...
    def _get_booked_item(self, obj):
        cache = getattr(self, "_booked_item_cache", {})
        if obj.pk not in cache:
            ...
            cache[obj.pk] = reservation.items.first() if reservation else None
            self._booked_item_cache = cache
        return cache[obj.pk]

    def get_booked_product_type(self, obj):
        item = self._get_booked_item(obj)
        return item.product_type if item else None

    def get_booked_package_db_id(self, obj):
        item = self._get_booked_item(obj)
        return item.package_db_id if item else None
