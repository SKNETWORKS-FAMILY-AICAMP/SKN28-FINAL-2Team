...
def preprocess_place_csv(input_path: str | Path, rules: PlaceRules) -> PlacePreprocessResult:
    ...
    return PlacePreprocessResult(
        ...
        dataset_counts=dict(sorted(Counter(doc["metadata"]["dataset"] for doc in documents).items())),
...
def classify_place_record(record: Mapping[str, Any], rules: PlaceRules) -> Classification:
...
def _classify_tourism_record(
    record: Mapping[str, Any], rules: PlaceRules
) -> Classification:
