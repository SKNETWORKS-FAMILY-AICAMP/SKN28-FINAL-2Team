...
from ..storage.config import MySQLConfig


DEFAULT_AIHUB_DATABASE = "tour_recommender_aihub"
...
def mysql_config_from_env() -> MySQLConfig:
    ...
    database = os.getenv("AIHUB_MYSQL_DATABASE", DEFAULT_AIHUB_DATABASE).strip()
    if not database or not database.replace("_", "").isalnum():
        raise ValueError(
            "AIHUB_MYSQL_DATABASE may contain only letters, numbers, and underscores"
        )
