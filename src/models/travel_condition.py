...
class ConditionDelta:
    ...
    def from_mapping(
        ...
        return cls(
            ...
            remove_must_visit_places=_string_tuple(
                value.get("remove_must_visit_places")
            ),
            add_excluded_places=_string_tuple(
                value.get("add_excluded_places")
            ),
            remove_excluded_places=_string_tuple(
                value.get("remove_excluded_places")
            ),
...
def apply_delta(
    ...
    must_visit = tuple(
        ...
        if place not in excluded
    )

    excluded = tuple(
        place
        for place in excluded
        if place not in must_visit
    )
...
def infer_affected_slots(
    delta: ConditionDelta,
) -> tuple[SlotRole, ...]:
    if delta.affected_slots:
        return tuple(
            dict.fromkeys(
                delta.affected_slots
            )
        )
    ...
    if (
        ...
        in delta.add_preferred_visit_types
        or any(
            "맛집" in place
            or "카페" in place
            or "식당" in place
    ...
    if any(
        preference
        not in (VisitPreference.FOOD_CAFE,)
        for preference in (
            *delta.add_preferred_visit_types,
            *delta.remove_preferred_visit_types,
        )
    ...
    if (
        delta.add_must_visit_places
        or delta.remove_must_visit_places
    ):
        roles.extend(
            [
                "visit",
...
def _slot_add_request_tuple(
    value: Any,
) -> tuple[SlotAddRequest, ...]:
    if not value:
        return ()
