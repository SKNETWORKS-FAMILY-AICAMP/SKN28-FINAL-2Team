...
def fetch_lcls_code_tree(
    *,
    ...
    while queue:
        ...
        if calls_used > call_budget:
            raise OpenApiError(f"lcls code lookup used {calls_used} calls, over budget {call_budget}.")
    ...
    return LclsCodeResult(records=all_records, calls_used=calls_used, max_depth=max_depth)
...
def _response_items(body: dict[str, Any]) -> list[dict[str, Any]]:
