...
현재 전처리 결과는 2,112개 문서이며 12개는 정책 제외 또는 완전 중복으로 RAG에서
제외됩니다. 원본 장소는 삭제하지 않고 MySQL에 모두 보존합니다.
